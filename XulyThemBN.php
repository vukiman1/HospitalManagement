...
if (isset($_POST["b1"])) {
    ...
    $existingPatient = getExistingPatient($mathe);

    if ($existingPatient) {
        $ketqua = UpdateBenhNhan($existingPatient['id'], $hoten, $tuoi, $gioitinh, $ngaysinh, $diachi, $mathe, $trangthai, $benh);
        if ($ketqua) {
            echo "<div class='text-center'>
                    <h1>CẬP NHẬT THÀNH CÔNG</h1>
                    <span>Thông tin bệnh nhân đã được cập nhật!</span>
                 </div>";
        } else {
            echo "<h3>LỖI CẬP NHẬT DỮ LIỆU</h3>";
        }
    } else {
        $ketqua = AddBenhNhan($hoten, $tuoi, $gioitinh, $ngaysinh, $diachi, $mathe, $trangthai, $benh);
        if ($ketqua) {
            echo "<div class='text-center'>
                    <h1>THÀNH CÔNG</h1>
                    <span>Bệnh nhân đã được thêm thành công!</span>
                 </div>";
        } else {
