...
<body>
    ...
    <?php
            ...
            $tuoi = $_REQUEST["tTuoi"];
            $gioitinh=$_REQUEST["tGioitinh"];
            
            $ngaysinh = $_REQUEST["tNgaysinh"];
            $diachi = $_REQUEST["tDiachi"];
            $mathe = $_REQUEST["tMathe"];
            $trangthai = $_REQUEST["tTrangThai"];
            $benh = $_REQUEST["tBenh"];
            $ketqua = UpdateBenhNhan($id, $hoten,$tuoi,$gioitinh,$ngaysinh,$diachi,$mathe, $trangthai, $benh);
            if($ketqua==TRUE)
                echo "<div class='text-center'>
                <h1>THÀNH CÔNG </h1>
                <span>Bệnh nhân đã được sửa thành công!</span>
            </div>";
            else
                echo "<div class='text-center'>
                <h1>Lỗi sửa dữ liệu </h1>
